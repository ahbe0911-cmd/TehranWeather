  ...
  Future<GeoPointInfo> locate() async {
    ...
    if (!serviceEnabled) {
      throw const LocationProblem(
        'سرویس موقعیت مکانی خاموش است. GPS را روشن کنید و دوباره تلاش کنید.',
      );
    }
    ...
    if (permission == LocationPermission.denied) {
      throw const LocationProblem(
        'برای نمایش آب‌وهوای دقیق، دسترسی موقعیت مکانی لازم است.',
      );
    }

    if (permission == LocationPermission.deniedForever) {
      throw const LocationProblem(
        'دسترسی موقعیت برای همیشه غیرفعال شده است. از تنظیمات Android آن را فعال کنید.',
    ...
    try {
      ...
      if (fallback == null) {
        throw const LocationProblem(
          'GPS نتوانست مختصات فعلی را دریافت کند. در فضای باز دوباره تلاش کنید.',
        );
      }
      position = fallback;
    }
...
class WeatherService {
  ...
  Future<WeatherSnapshot> fetch(GeoPointInfo location) async {
    ...
    final current = CurrentWeather(
      ...
      precipitation: _double(currentJson['precipitation']),
    ...
    for (var i = 0; i < times.length; i++) {
      ...
      hourly.add(HourlyWeather(
        time: time,
        temperature: _double(temps[i]),
        precipitationProbability: _int(pops[i]),
        weatherCode: _int(codes[i]),
        windSpeed: _double(winds[i]),
      ));
    ...
    for (var i = 0; i < dates.length; i++) {
      daily.add(DailyWeather(
        ...
        maxTemperature: _double(maxTemps[i]),
        minTemperature: _double(minTemps[i]),
        precipitationProbability: _int(dailyPops[i]),
        weatherCode: _int(dailyCodes[i]),
...
class WeatherCache {
  static const _key = 'last_weather_snapshot_v2';
